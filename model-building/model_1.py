...
def train_model(X_train,y_train):
    ...
    model = clone(model)#Clone uses exact same hyperparameters that were used to train the model but does not attach any training details(basically trains the model from scratch)
...
def promote_model(y_val,y_proba,version):
    ...
    if pr_auc_for_latest_model>pr_auc_for_champion_model:
        client.set_registered_model_alias(name=registered_model_name,alias="Champion",version=version)
