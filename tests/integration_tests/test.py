...
def test_model_api():
    ...
    transaction = {'Time': 154309.0,
     ...
     'Amount': 1096.99}
    result = requests.post(url,json = transaction)
    fraud = result.json()['fraud']
    probability_of_fraud = result.json()['probability_of_fraud']

    assert fraud in [0,1]
    assert probability_of_fraud>=0 and probability_of_fraud<=1

def test_api_health():
    url = "http://localhost:9696/health"
    result = requests.get(url)
    status_code = result.json()['response_status']
