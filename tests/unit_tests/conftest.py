import joblib
import pandas as pd
import pytest

#Important-> pytest fixture is used when a method or resource is used in all the files
# You do not import it , rather define it as a pytest fixture and when(in other files)
# you pass the name of that fixture as argument it is called directly 
@pytest.fixture
def model():
    return joblib.load('./fraud_model.joblib')

@pytest.fixture
def transaction():
    return pd.DataFrame([{'Time': 154309.0,
     'V1': -0.0829834650336291,
     'V2': -3.93591892431521,
     'V3': -2.6167089860612,
     'V4': 0.163310074791927,
     'V5': -1.40095182767067,
     'V6': -0.809419404822953,
     'V7': 1.50157983974791,
     'V8': -0.47099958719182,
     'V9': 1.51974319879361,
     'V10': -1.13445381068391,
     'V11': 0.71387764641859,
     'V12': 0.979674835308759,
     'V13': -1.33903126624009,
     'V14': 0.984993219111346,
     'V15': -0.382470968292812,
     'V16': -0.934615796103053,
     'V17': -0.0524194348357059,
     'V18': 0.51116178431397,
     'V19': 0.731998749113651,
     'V20': 1.87861160253024,
     'V21': 0.70267203994139,
     'V22': -0.182304775613767,
     'V23': -0.921016986180311,
     'V24': 0.111635488151138,
     'V25': -0.0716222119619373,
     'V26': -1.12588109590433,
     'V27': -0.170947427341269,
     'V28': 0.126221382847378,
     'Amount': 1096.99}])

@pytest.fixture
def input_columns():
    return ['Time', 'V1', 'V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10',
       'V11', 'V12', 'V13', 'V14', 'V15', 'V16', 'V17', 'V18', 'V19', 'V20',
       'V21', 'V22', 'V23', 'V24', 'V25', 'V26', 'V27', 'V28', 'Amount']