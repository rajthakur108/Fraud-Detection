...
report = Report(metrics=[ColumnDriftMetric(column_name='prediction'),DatasetDriftMetric(),
...
def calculate_metric_postgres(curr,i,start_day):
    ...
    share_of_missing_values = results['metrics'][2]['result']['current']['share_of_missing_values']
    fraud_prediction_rate = data["prediction"].mean()
    fraud_prediction_proba_avg = data['fraud_probability'].mean()


    curr.execute("insert into fraud_metrics (timestamp,prediction_drift,num_drifted_columns,share_missing_values,fraud_prediction_rate,fraud_prediction_proba_avg) values(%s, %s, %s, %s, %s, %s)",
                 (begin+datetime.timedelta(start_day),drift_score,num_drifted_columns,share_of_missing_values,fraud_prediction_rate,fraud_prediction_proba_avg))
